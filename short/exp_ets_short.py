...
df_results.to_parquet(f"results/{model_name}-short-time.parquet", index=False)

df_forecastings = pd.DataFrame(forecastings)
df_forecastings.to_parquet(f"results/forecasting-{model_name}-short-time.parquet", index=False)
